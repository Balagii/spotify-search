...
def main(argv=None):
    ...
    parser.add_argument(
        "name", help="Exact playlist name (use --contains for substring match)"
    )
    parser.add_argument("-o", "--output", help="Output file path (defaults to stdout)")
    parser.add_argument(
        "--contains", action="store_true", help="Match substring instead of exact name"
    )
    parser.add_argument(
    ...
    try:
        ...
        pid, matches = find_playlist_id_by_name(
            client, args.name, contains=args.contains
        )
        if not pid:
            if matches:
                print(
                    "Multiple playlists matched. Be more specific or use --contains:",
                    file=sys.stderr,
                )
                for p in matches:
                    print(
